...
export type AdminStats = {
  ...
  recentUsers: { email: string; name: string; email_verified: boolean; created_at: string }[];
};

/** Aggregated account stats for the admin dashboard. Admin-gated by the caller. */
export async function getAdminStats(): Promise<AdminStats> {
  ...
  const recentUsers = await sql`
    SELECT email, name, email_verified, created_at FROM users ORDER BY created_at DESC LIMIT 20
  `;

  return {
    totalUsers: totals.total,
    verifiedUsers: totals.verified,
    lockedUsers: totals.locked,
    signupsByDay: signupsByDay as AdminStats["signupsByDay"],
